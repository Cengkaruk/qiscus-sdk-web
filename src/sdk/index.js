...
export class qiscusSDK extends EventEmitter {
  ...
  */
    ...
    return self.roomAdapter.getRoomById(id)
      .then((response) => {
        ...
        if (!roomToFind) {
          ...
          roomToFind.last_comment_id = roomToFind.comments[roomToFind.comments.length-1].id
          room = roomToFind
        }
        self.last_received_comment_id = (self.last_received_comment_id < room.last_comment_id) ? room.last_comment_id : self.last_received_comment_id
  ...
  getNonce() {
    ...
    return request.set('qiscus_sdk_app_id', `${this.AppId}`).post(`${this.baseURL}/api/v2/sdk/auth/nonce`)
      .send()
  ...
  verifyIdentityToken(identity_token) {
    return request.set('qiscus_sdk_app_id', `${this.AppId}`).post(`${this.baseURL}/api/v2/sdk/auth/verify_identity_token`)
      .send({identity_token})
      .then(res => Promise.resolve(res.body.results), 
        err => Promise.reject(err));
  }
